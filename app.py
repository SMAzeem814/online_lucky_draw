...
def inject_now():
    return {'now': datetime.now}   # NOW() for reports
...
def safe_cursor(conn):
    try:
        ...
        return conn.cursor(cursor_class=mysql.connector.cursor.MySQLCursorDict)


def send_winner_email(user_email, user_name, draw_title, draw_date):
    """
    Send congratulation email using Gmail SMTP
    """
    try:
        ...
        logging.exception("Failed to send Gmail winner email: %s", e)
...
def register():
    if request.method == "POST":
        ...
        hashed = generate_password_hash(password) if HAVE_WERKZEUG else password
    ...
    if request.method == "POST":
        ...
        if participant:
            ...
            """, (name, email, phone, payment_method,
                  bank_name, amount, draw_id, user_id))
        else:
            cur.execute("""
            ...
            """, (user_id, draw_id, name, email, phone,
                  payment_method, bank_name, amount))
